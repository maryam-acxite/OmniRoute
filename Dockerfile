...
RUN --mount=type=cache,id=s/92ca8a61-c1ba-421f-a389-d48ac7258c2d-next-cache,target=/app/.build/next/cache \
  mkdir -p /app/data \
  && npm run build \
  && cp -r node_modules/@atjsh node_modules/@huggingface node_modules/onnxruntime-node node_modules/js-tiktoken .build/next/standalone/node_modules/ 2>/dev/null || true
# ── Runner base ────────────────────────────────────────────────────────────
